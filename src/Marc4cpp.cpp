...
//			leader->setPos9Undefined(BLANK);	// 9, typeOfEntity

			// pos. 17 livelli 90-97 con blank, livelli 05-71 con ‘3’ Roveri Jira 29/05/20
			int livelloAuthority = atoi (tbTitolo->getField(tbTitolo->cd_livello));
			if (livelloAuthority >= 90 && livelloAuthority <= 97)
				leader->setLivelloDiCodifica(' ');
			else if (livelloAuthority >= 5 && livelloAuthority <= 71)
				leader->setLivelloDiCodifica('3');
