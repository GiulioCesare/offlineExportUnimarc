...
DataField * Marc4cppDocumento::creaTag300_NoteGenerali(char *areaStartPtr, char *areaEndPtr) {
	...
	// 17/04/2020 Rifatto per gestire le note con le URL (Mataloni/Scognamiglio et al)
	CString note = areaStartPtr;

	ATTValVector<CString *> noteVect;
	int retb = note.Split(noteVect, ". - ");

	CString *notaPtr;
	for (int i=0; i < noteVect.Length(); i++ )
	{
//		if (i==0)
//		{ // For testing
//			note="<URL>commento1 | http://url1 ; http://url2";
//			notaPtr=&note;
//		}
//		else
			notaPtr=noteVect.Entry(i);

		if ((DATABASE_ID == DATABASE_INDICE) && (notaPtr->StartsWith("<URL>")))
//		if ( notaPtr->StartsWith("<URL>"))
		{
			notaPtr->CropLeftBy(5); // Remove URL tag
			ATTValVector<CString *> urlsVect;
			ATTValVector<CString *> linkVect;
			CString *linkPtr;

			int retb = notaPtr->Split(urlsVect, " ; ");

			for (int i=0; i < urlsVect.Length(); i++ )
			{
				linkPtr=urlsVect.Entry(i);
				int retb = linkPtr->Split(linkVect, " | ");

				df = new DataField("399", 3);
				if (linkVect.length() > 1)
				{
					sf = new Subfield('a', linkVect.Entry(0));
					df->addSubfield(sf);
					sf = new Subfield('b', linkVect.Entry(1));
					df->addSubfield(sf);
				}
				else
				{
					sf = new Subfield('b', linkVect.Entry(0));
					df->addSubfield(sf);
				}
				linkVect.DeleteAndClear();

				marcRecord->addDataField(df);

			} // End for urlsVect

			urlsVect.DeleteAndClear();

		}
		else
		{
			df = new DataField("300", 3);
			sf = new Subfield('a', notaPtr);
			df->addSubfield(sf);
			marcRecord->addDataField(df);
		}
...
//		    key = getPolo(); // prendi il polo
		    key = trTitBib->getFieldString(trTitBib->cd_polo)->Data(); // 10/10/2019 prendi il polo
...
//tbNota300->dumpRecord();
	...
	char *areaStartPtr = s.data();
		char *areaEndPtr = areaStartPtr+s.Length();
		creaTag300_NoteGenerali(areaStartPtr, areaEndPtr); // 24/04/2020 per gestione 399 (URL)
/*
	//	CString s = areaStartPtr;
	long start = 0;
