...
//		keyPlusOffsetPlusLfLength = saveKeyPlusOffsetPlusLfLength;
	...
	if (trRepTitOffsetIn)
	{
		trRepTitOffsetIn->SeekToEnd();
		fileSize = trRepTitOffsetIn->CurOffset();
		elements = elementsTrRepTit = fileSize/keyPlusOffsetPlusLfLength;
		if (offsetFileVectorInMem.FindByValue(trRepTitOffsetIn) != -1)
		{
			printf ("\nCarico in memoria indice %s, keyOffsetLen=%d, elements=%ld", trRepTitOffsetIn->GetName(), keyPlusOffsetPlusLfLength, elements);
			offsetBufferTrRepTitPtr = (char *)malloc(elements*(keyPlusOffsetPlusLfLength)); // n righe di classe+Offset
			if (!offsetBufferTrRepTitPtr)	{
				printf ("\n!!!! ALLOCAZIONE MEMORIA FALLITA");
				return false;	}
			loadOffsetFiles2(trRepTitOffsetIn, offsetBufferTrRepTitPtr);
		}
	}


	// Carichiamo gli offset delle relazioni marche/repertori
	if (trRepMarOffsetIn)
	{
...
//		if (loadInMemory && (*loadInMemory =='N' || *loadInMemory =='n'))
		...
		else if (!strcmp(tabella, "tr_rep_tit_off")) // 08/06/2020
			trRepTitOffsetIn = cFileIn;
...
printf ("\nAPRI FILE ENTITA': %s", sPtr->data());
		...
		if (!strcmp(tabella, "tb_titolo"))
			...
			trRepAutIn = cFileIn;
		else if (!strcmp(tabella, "tr_rep_tit"))
			trRepTitIn = cFileIn;
