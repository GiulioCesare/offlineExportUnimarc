...
TbfBibliotecaInPolo::TbfBibliotecaInPolo(CFile *tbIn, CFile *tbOffsetIn, char *offsetBufferTbPtr, long elementsTb, int keyPlusOffsetPlusLfLength, int key_length) :
...
TbfBibliotecaInPolo::TbfBibliotecaInPolo() : Tb()
{
	tbFields = 14;
	init();
}
...
bool TbfBibliotecaInPolo::loadRecord(const char *key)
{

	if (!Tb::loadRecord(key))
	{
//		SignalAnError(__FILE__, __LINE__, "Derived class TbfBibliotecaInPolo::loadRecord");
