...
void CMisc::formatDate_dd_mm_yyyy(struct tm * timeinfo, char *destDate) { // 24/06/2020

	char tmpBuf[10];
	sprintf(destDate, "%02d.", timeinfo->tm_mday);
	destDate += 3;

	sprintf(tmpBuf, "%02d.", timeinfo->tm_mon);
	memcpy(destDate, tmpBuf, 3);
//	*((short*)destDate) = *((short*)tmpBuf);
	destDate += 3;

	sprintf(tmpBuf, "%04d", timeinfo->tm_year + 1900);
	memcpy(destDate, tmpBuf, 4);
//	*((int*)destDate) = *((int*)tmpBuf);
//	destDate += 4;
