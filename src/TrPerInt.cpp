...
TrPerInt::TrPerInt(CFile *tbIn, CFile *tbOffsetIn, char *offsetBufferTbPtr, long elementsTb, int keyPlusOffsetPlusLfLength, int key_length) :
...
bool TrPerInt::loadRecord(const char *key)
{
	if (!Tb::loadRecord(key))
	{
//		SignalAnError(__FILE__, __LINE__, "Derived class TrPerInt::loadRecord");
